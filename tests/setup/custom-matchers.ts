...
expect.extend({
	...
	async toHaveSessionForUser(response: Response, playerId: string) {
		...
		const session = await prisma.session.findUnique({
			select: { id: true },
			where: { playerId, id: sessionValue },
		})
		...
		return {
			...
			message: () =>
				`A session was${
					this.isNot ? ' not' : ''
				} created in the database for ${playerId}`,
		}
	},
...
interface CustomMatchers<R = unknown> {
	...
	toHaveSessionForUser(playerId: string): Promise<R>
